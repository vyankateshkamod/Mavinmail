...
import { User, LogOut, CheckCircle2, AlertCircle, RefreshCw, Save, Mail, Calendar, Clock, Zap, Crown, Coins, Database, Download } from 'lucide-react';
import { getUserStats, getConnectionStatus, updateUserProfile, getUserProfile, getUserCredits, upgradeToPro, topUpCredits, type SyncResult, type SyncStatusData } from '../services/api';

interface ProfileScreenProps {
  onLogout: () => void;
  // Sync props from useSyncManager
  isSyncing?: boolean;
  syncStatus?: SyncStatusData | null;
  lastSyncResult?: SyncResult | null;
  syncError?: string | null;
  onTriggerSync?: (count: number, afterDate?: string) => Promise<any>;
...
export default function ProfileScreen({
  ...
  const [localSyncMessage, setLocalSyncMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);
    ...
    <div className="flex h-screen w-full flex-col bg-gray-50 dark:bg-[#0A0A0A] text-gray-900 dark:text-white font-sans selection:bg-[#22d3ee] selection:text-black">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto p-5 scrollbar-thin scrollbar-thumb-[#22d3ee]/20 scrollbar-track-transparent">
        <div className="max-w-md mx-auto space-y-8 pb-10">
          ...
          {/* 4.5 Email Sync Card */}
          <div className="bg-white dark:bg-[#121212] border border-gray-300 dark:border-[#262626] rounded-xl p-4 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="p-1.5 bg-[#22d3ee]/10 rounded-lg">
                  <Database className="w-4 h-4 text-[#22d3ee]" />
                </div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Email Sync</h3>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-50 dark:bg-[#0A0A0A] rounded-lg p-2.5">
                <p className="text-[10px] text-gray-500 uppercase tracking-wider">Emails Synced</p>
                <p className="text-lg font-bold text-gray-900 dark:text-white mt-0.5">
                  {syncStatus?.totalEmailsSynced ?? 0}
                </p>
              </div>
              <div className="bg-gray-50 dark:bg-[#0A0A0A] rounded-lg p-2.5">
            ...
            <div className="space-y-2.5">
              <div className="flex gap-2">
                <select
                  ...
                  disabled={isSyncing}
                  className="flex-1 bg-gray-50 dark:bg-[#0A0A0A] border border-gray-300 dark:border-[#333] rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-[#22d3ee] transition-colors appearance-none cursor-pointer disabled:opacity-50"
                >
                ...
                <input
                  ...
                  disabled={isSyncing}
                  className="flex-1 bg-gray-50 dark:bg-[#0A0A0A] border border-gray-300 dark:border-[#333] rounded-lg px-3 py-2 text-xs text-gray-900 dark:text-white focus:outline-none focus:border-[#22d3ee] transition-colors disabled:opacity-50"
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (result) {
                      setLocalSyncMessage({
                        type: 'success',
                        text: `Synced ${result.synced} new emails (${result.skipped} already synced)`,
                      });
                    }
                  } catch (err: any) {
                    setLocalSyncMessage({
                      type: 'error',
                      text: err?.response?.data?.message || 'Sync failed',
                    });
                  }
                }}
                disabled={isSyncing || !onTriggerSync}
                className="w-full bg-[#22d3ee]/10 hover:bg-[#22d3ee]/20 text-[#22d3ee] border border-[#22d3ee]/20 py-2.5 rounded-xl font-medium transition-all text-xs flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSyncing ? (
                  <>
            ...
            {(localSyncMessage || syncError || lastSyncResult) && (
              <div
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-[11px] ${
                  localSyncMessage?.type === 'error' || syncError
              ...
              >
                ...
                <span>
                  {localSyncMessage?.text ||
                    syncError ||
                    (lastSyncResult &&
                      `${lastSyncResult.synced} synced, ${lastSyncResult.skipped} skipped`)}
                </span>
              </div>
            )}

            <p className="text-[10px] text-gray-500 leading-relaxed">
              Emails are automatically synced in the background. Duplicates are never re-embedded.
            </p>
          </div>
